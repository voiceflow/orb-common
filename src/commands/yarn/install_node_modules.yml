  ...
  - when:
      ...
      steps:
        - yarn_command:
            ...
            # `bash -c` in run_command.sh does not inherit pipefail, so without this the pipeline
            # reports tee's exit status and a failed install passes silently, poisoning the cache
            yarn_command: set -o pipefail; yarn install --immutable << parameters.install_args >> 2>&1 | tee output.file
  - unless:
      condition: << parameters.avoid_post_install_scripts >>
      steps:
        - yarn_command:
