  ...
  - when:
      ...
      steps:
        - yarn_command:
            ...
            yarn_command: yarn --frozen-lockfile --ignore-scripts << parameters.install_args >> 2>&1 | tee output.file
  - unless:
      condition: << parameters.avoid_post_install_scripts >>
      steps:
        - yarn_command:
            ...
            step_name: << parameters.step_name >>
            wait: << parameters.wait >>
            yarn_command: yarn --frozen-lockfile  << parameters.install_args >>
  - run:
      working_directory: << parameters.working_directory >>
      name: Check yarn lock integrity
      command: <<include(scripts/yarn/check-lock.sh)>>
